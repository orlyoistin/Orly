...
class Controller_Admin_Disposisi extends Controller_Admin_Backend {
	...
	public function action_save() {	
		...
		try {
			...
			if(isset($_POST['sotk'])) {
				foreach($_POST['sotk'] as $sotk) {
					DB::insert('notes', array('tanggal', 'sotk_id', 'disposisi_id', 'masterbool_id'))
			...
			$view = View::factory('admin/disposisi_form')
				->bind('errors',$errors)
				->set('form',$_POST)	
				->set('url',URL::base().'admin/disposisi/save')
				...
				->set('submit_value',"Tambah Data")
				->set('wakil_list',$this->getList('Wakil','name','1'))
				->set('sotk_list',$this->getList('Sotk','name','1',array(array('skpd_id','=',Auth::instance()->get_user()->skpd->id))));
