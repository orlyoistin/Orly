...
class Controller_Admin_Skpd extends Controller_Admin_Backend {
	public $auth_required = array('login');
			
	public function action_index() {				
		$this->template->content = View::factory('admin/skpd');
	}
	
	public function action_new() {
		$this->auto_render = false;
		
		echo View::factory('admin/skpd_form')
			->bind('errors',$error)
			->set('form',$this->getField('Skpd'))										
			->set('url',URL::base().'admin/skpd/create')
	...
	public function action_edit() {
		...
		echo View::factory('admin/skpd_form')
			->bind('errors',$errors)
			->set('form',$form)
			->set('url',URL::base().'admin/skpd/update/'.$id)
	...
	public function action_update() {
		...
		try {
			...
			$errors = $e->errors('models');			
			$view = View::factory('admin/skpd_form')
				->bind('errors', $errors)
				->bind('form', $_POST)
				->set('url',URL::base().'admin/skpd/update/'.$id)
				->set('submit_value',"Update Data")
	...
	public function action_data() {
		...
		foreach($viewskpds as $viewskpd) {
			...
			$arrData[] = array(
				$i + $_REQUEST['start'],
				"<a title='SOTK' class='btn btn-warning btn-xs' href='".URL::base()."admin/sotk/index/".$skpd->id."'><i class='fa fa-sitemap'></i></a>",
				$skpd->kode,
				"<a data-fancybox data-type='ajax' data-src='".URL::base()."admin/skpd/edit/".$skpd->id."'>".$skpd->name."</a>",
				$distribution
			);
