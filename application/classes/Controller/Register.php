...
class Controller_Register extends Controller_Layout {	
	...
	public function action_login() {
		$this->template->content = View::factory('login')
			...
			->set('url',URL::base().'register/login')
			->bind('submit_value',$submit_value);		
					
		if(HTTP_Request::POST == $this->request->method()) {
			if(Captcha::valid($_POST['security_code'])) {
				...
				if($user) {
					...
					HTTP::redirect(URL::base().'register/login');
				}
			}
			else {
				HTTP::redirect(URL::base().'register/login');
	...
	public function action_add() {
		...
		$model->add('roles', ORM::factory('Role')->where('name', '=', 'root')->find());
	}
}