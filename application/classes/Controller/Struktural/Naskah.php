...
class Controller_Struktural_Naskah extends Controller_Struktural_Backend {
	public $auth_required = array('login','struktural');
	
	function action_index(){											
		$this->template->content = View::factory('struktural/naskah');
	}
	...
	public function action_new() {
		...
		$this->template->content = View::factory('struktural/naskah_form')
    		->set('form',$this->getField('Naskah'))
    		->set('url',URL::base().'struktural/naskah/save')										
	...
	public function action_edit() {
		...
		$this->template->content = View::factory('struktural/naskah_form')
			->bind('form',$form)
			->set('id',$id)
			->set('url',URL::base().'struktural/naskah/update/'.$id)
	...
	public function action_save() {	
		try {
			...
			HTTP::redirect(URL::base().'struktural/naskah');			
		}
	...
	public function action_update() {	
		...
		try {
			...
			$revision = $naskah->revision
				->where('kepada','=',Auth::instance()->get_user()->sotk_id)
				->find();
			...
			if(isset($_POST['delete'])==1) {
				...
				$kepada = $revision->dari;
				if($_POST['mastersurat_id'] == 3) {
					$sotk = ORM::factory('Sotk',Auth::instance()->get_user()->sotk_id);

					$kepada = $sotk->parent_id;
				}
				elseif($_POST['mastersurat_id'] == 4) {
					$kepada = Auth::instance()->get_user()->sotk_id;
				}
				
				$field = array('naskah_id', 'dari', 'kepada', 'tanggal', 'catatan', 'mastersurat_id', 'status_baca', 'created', 'user_id');
				$value = array($naskah->id, Auth::instance()->get_user()->sotk_id, $kepada, date("Y-m-d H:i:s"), $_POST['catatan'], $_POST['mastersurat_id'], 1, date("Y-m-d H:i:s"), Auth::instance()->get_user()->id);	
	...
	public function action_view() {
		...
		echo View::factory('struktural/naskah_view')
			->bind('naskah',$naskah);
	}
	...
	public function action_revision() {	
		...
		echo View::factory('struktural/revision')
	...
	public function action_data() {
		...
		foreach($viewnaskahs as $viewnaskah) {
			...
			$arrData[] = array(
				$i + $_REQUEST['start'],
				"<a data-fancybox data-type='ajax' data-src='".URL::base()."struktural/naskah/revision/".$viewnaskah->id."' class='btn btn-primary btn-xs' title='Revision'><i class='fa fa-sort-amount-asc'></i></a>",
			    $tanggal->format("d-m-Y"),
				"<a href='".URL::base()."struktural/naskah/edit/".$viewnaskah->id."'>".$viewnaskah->nomor."</a>",
				$viewnaskah->perihal,
